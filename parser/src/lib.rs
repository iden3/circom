...
pub fn run_parser(file: String) -> Result<(ProgramArchive, ReportCollection), (FileLibrary, ReportCollection)> {
    ...
    while let Some(crr_file) = FileStack::take_next(&mut file_stack) {
        ...
        warnings.append(&mut check_number_version(path, program.compiler_version ).map_err(|e| (file_library.clone(), vec![e]))?);
    }
...
fn check_number_version(file_path: String, version: Option<Version>) -> Result<ReportCollection, Report>{
    use errors::{CompilerVersionError, NoCompilerVersionWarning};
    if let Some(required_version) = version {
        if required_version == (2,0,0){
            Ok(vec![])
        }
        else{
            let report = CompilerVersionError::produce_report(CompilerVersionError{path: file_path, required_version: required_version, version: (2,0,0)});
            Err(report)
        }
    }
    else{
        let report = NoCompilerVersionWarning::produce_report(NoCompilerVersionWarning{path: file_path, version: (2,0,0)});
        Ok(vec![report])
    }
}