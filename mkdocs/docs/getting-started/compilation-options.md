...
In the following, we explain some of these options.
...
* Finally, flag ```--use_old_simplification_heuristics``` allows to use a previous heuristics of the optimization algorithm. However, it is not recommended since the new heuristics has produced better results in practice.
