...
use inkwell::values::{AnyValue, AnyValueEnum, BasicMetadataValueEnum, BasicValue, BasicValueEnum, FunctionValue, InstructionValue, IntMathValue, IntValue, PhiValue, PointerValue};
...
// if (rhs < 0)
//   res = 1/res;
pub fn create_pow_with_name<'a, T: IntMathValue<'a> + Copy>(producer: &dyn LLVMIRProducer<'a>, in_func: FunctionValue<'a>, lhs: T, rhs: T, name: &str) -> AnyValueEnum<'a> {
    let bldr = &producer.llvm().builder;
    let ty = bigint_type(producer);

    //// Generate absolute value if-then-else
    let res_is_neg = create_call(producer, FR_LT_FN_NAME, 
    ...
    //// Generate loop to perform multiplications
    {
        ...
        let res_cond = create_lt(producer, 
        ...
        let cond = create_conditional_branch(producer,
            res_cond.into_int_value(),
            bb_lp_body, bb_lp_end
        );

        //// Loop body block
        producer.set_current_bb(bb_lp_body);
        //   store( res, fn_mul( load(res) , lhs )) -- use method for finite field operation
        let res_mul = create_call(producer, FR_MUL_FN_NAME, 
        ...
        let res_add = create_add(producer, 
            bldr.build_load(ptr_lp_var, "").into_int_value(),
    ...
    //// Generate if-then to invert result when exponent is negative
    {
        ...
        // Then branch
        producer.set_current_bb(bb_if_then);
        bldr.build_store(ptr_res, create_inv(producer, bldr.build_load(ptr_res, "").into_int_value()).into_int_value());
        bldr.build_unconditional_branch(bb_if_merge);
        // Merge/tail block
        producer.set_current_bb(bb_if_merge);
    }
...
pub fn create_pow<'a, T: IntMathValue<'a> + Copy>(producer: &dyn LLVMIRProducer<'a>, in_func: FunctionValue<'a>, lhs: T, rhs: T) -> AnyValueEnum<'a> {
...
pub fn create_logic_and_with_name<'a, T: IntMathValue<'a>>(producer: &dyn LLVMIRProducer<'a>, lhs: T, rhs: T, name: &str) -> AnyValueEnum<'a> {
    producer.llvm().builder.build_and(lhs, rhs, name).as_any_value_enum()
}

pub fn create_logic_and<'a, T: IntMathValue<'a>>(producer: &dyn LLVMIRProducer<'a>, lhs: T, rhs: T) -> AnyValueEnum<'a> {
    create_logic_and_with_name(producer, lhs, rhs, "")
}

pub fn create_logic_or_with_name<'a, T: IntMathValue<'a>>(producer: &dyn LLVMIRProducer<'a>, lhs: T, rhs: T, name: &str) -> AnyValueEnum<'a> {
    producer.llvm().builder.build_or(lhs, rhs, name).as_any_value_enum()
}

pub fn create_logic_or<'a, T: IntMathValue<'a>>(producer: &dyn LLVMIRProducer<'a>, lhs: T, rhs: T) -> AnyValueEnum<'a> {
    create_logic_or_with_name(producer, lhs, rhs, "")
}

pub fn create_logic_not_with_name<'a, T: IntMathValue<'a>>(producer: &dyn LLVMIRProducer<'a>, val: T, name: &str) -> AnyValueEnum<'a> {
    producer.llvm().builder.build_not(val, name).as_any_value_enum()
}

pub fn create_logic_not<'a, T: IntMathValue<'a>>(producer: &dyn LLVMIRProducer<'a>, val: T) -> AnyValueEnum<'a> {
