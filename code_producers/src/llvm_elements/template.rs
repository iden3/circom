...
use inkwell::values::{ArrayValue, FunctionValue, GlobalValue, IntValue, PointerValue};
...
impl<'a, 'b> TemplateLLVMIRProducer<'a, 'b> {
    pub fn new(parent: &'b dyn LLVMIRProducer<'a>, stack_depth: usize, number_subcmps: usize, current_function: FunctionValue<'a>) -> Self {
        TemplateLLVMIRProducer {
            parent,
            template_ctx: TemplateCtx::new(parent, stack_depth, number_subcmps, current_function),
        }
    }
}
...
impl<'a> TemplateCtx<'a> {
    pub fn new(producer: &dyn LLVMIRProducer<'a>, stack_depth: usize, number_subcmps: usize, current_function: FunctionValue<'a>) -> Self {
        ...
        let subcmps = create_alloca(producer, subcmps_ty.as_any_type_enum(), "subcmps").into_pointer_value();
        TemplateCtx {
            stack,
            subcmps,
            current_function
